...
>>>>>>> origin/master:src/edu/up/cs301/quoridor/QDComputerPlayer2.java
{
	...
	protected void receiveInfo(GameInfo info) {
		...
		// cast it
		if (!(info instanceof QDState)) return;
		QDState myState = (QDState)info;
		...
		if (win != null) {
			game.sendAction(new QDMovePawnAction(this, win.y, win.x));
			return;
		}
		...
		if (loss != null) {
			game.sendAction(new QDMovePawnAction(this, loss.y, loss.x));
			return;
		}
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (myState.getPiece(j, i) == ' ') {
					if (selectCount == 0) {
						// make the move
						game.sendAction(new QDMovePawnAction(this, j, i));
						return;
					}
					selectCount--;
				}
	 ...
	 */
	private Point findWin(QDState state, char thePiece) {
	...
	private Point helpFindWin(QDState state, char thePiece, int rowStart,
